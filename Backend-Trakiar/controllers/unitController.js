...
const createUnit = async (req, res) => {
  ...
  if (!identificador || !String(identificador).trim()) {
    return res
      .status(400)
      .json({ error: "El identificador de la unidad es obligatorio" });
  }
  ...
  try {
    ...
    if (!gerenteLinea) {
      return res
        .status(403)
        .json({ error: "No tienes una línea asignada para crear unidades" });
    }

    const unidadCreada = await pool.query(
      "INSERT INTO unidad (identificador, id_linea, estado, modified_by) VALUES ($1, $2, $3, $4) RETURNING *",
      [
    ...
    res
      ...
      .json({
        ...
        unidad: unidadCreada.rows[0],
      });
  } catch (error) {
    if (error.code === "22P02") {
      return res
        .status(400)
        .json({ error: "El estado de la unidad es inválido" });
    }

    if (error.code === "23505") {
      return res
        .status(400)
        .json({ error: "Ya existe una unidad con ese identificador" });
    }
...
const assignUnitToDriver = async (req, res) => {
  ...
  try {
    ...
    if (!unidad) {
      return res
        .status(404)
        .json({ error: "Unidad no encontrada o no pertenece a tu línea" });
    }

    if (unidad.id_chofer !== null) {
      return res
        .status(400)
        .json({ error: "La unidad ya está asignada a otro chofer" });
    }
    ...
    if (!chofer) {
      return res
        .status(404)
        .json({ error: "No se encontró un chofer asociado a ese correo" });
    }

    const idChofer = chofer.id;

    const unidadChofer = await pool.query(
      "SELECT id FROM unidad WHERE id_chofer = $1",
      [idChofer],
    );
    if (unidadChofer.rows.length > 0) {
      return res
        .status(400)
        .json({ error: "El chofer ya tiene una unidad asignada" });
    }

    // Asignar la unidad al chofer
    await pool.query(
      "UPDATE unidad SET id_chofer = $1, modified_by = $2 WHERE id = $3",
      [idChofer, req.user.id, idUnidad],
    );
...
const updateUnitDriver = async (req, res) => {
  ...
  try {
    ...
    if (unidad.id_chofer === chofer.id) {
      return res
        .status(200)
        .json({ message: "La unidad ya está asignada a ese chofer" });
    }

    const unidadChofer = await pool.query(
      "SELECT id FROM unidad WHERE id_chofer = $1 AND id <> $2",
      [chofer.id, idUnidad],
    );
    if (unidadChofer.rows.length > 0) {
      return res
    ...
    await pool.query(
      "UPDATE unidad SET id_chofer = $1, modified_by = $2 WHERE id = $3",
      [chofer.id, req.user.id, idUnidad],
    );

    return res
      .status(200)
      .json({ message: "Chofer de la unidad actualizado exitosamente" });
  } catch (error) {
    console.error(error);
    return res
      .status(500)
      .json({ error: "Error al actualizar el chofer de la unidad" });
  }
};
...
const updateUnitStatus = async (req, res) => {
  ...
  if (!idUnidad || !estado) {
    return res
      .status(400)
      .json({ error: "El id de unidad y el estado son obligatorios" });
  }
  ...
  try {
    ...
    await pool.query(
      "UPDATE unidad SET estado = $1, modified_by = $2 WHERE id = $3",
      [estado, req.user.id, idUnidad],
    );
    return res
      .status(200)
      .json({ message: "Estado de la unidad actualizado exitosamente" });
  } catch (error) {
    if (error.code === "22P02") {
      return res
        .status(400)
    ...
    return res
      .status(500)
      .json({ error: "Error al actualizar el estado de la unidad" });
  }
};
